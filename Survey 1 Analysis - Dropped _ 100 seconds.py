...
# ### Drop rows for responses that took less than 100 seconds to complete
...
for i in range(2, len(df.index)):
    if int(df.iloc[i, 5]) < 100: 
        drop_100.append(i)
df.drop(drop_100, inplace=True)
...
# Export edited data 
df.to_csv('Data/results_edited_dropped_100.csv', index=False)
...
# Drop Columns
df = pd.read_csv("Data/results_edited_dropped_100.csv", delimiter=",")
...
# Export edited data 
df.to_csv('Data/results_no_metadata_dropped_100.csv', index=False)
...
df = pd.read_csv("Data/results_no_metadata_dropped_100.csv", delimiter=",", dtype=int)
...
def plot_figs(index, means, sds, standard, file_name):
    ...
    plt.bar(index, means, yerr=sds, align='center', alpha=0.8, ecolor='black', capsize=10, color='tab:orange')
    plt.ylabel('Rating of Facebook Posts')
    plt.xticks(index)
    plt.title('Means and Standard Devations for %s (Response Time > 100 sec)' % standard)
    plt.tight_layout()
    plt.savefig('Figures_dropped_100/%s_bar_plot_with_error_bars_dropped_100.png' % file_name, overwrite=True)
    plt.show()
...
def create_stat_charts(index, means, sds, standard, file_name):
    t = Table([index, means, sds], names=('index', 'means', 'sds'), meta={'name': 'Table on Survey 1 %s Means and Variances (Response Time > 100 seconds)' % file_name})
    ascii.write(t, 'Tables_dropped_100/table_of_%s_means_sds_dropped_100.csv' % file_name, format='csv', fast_writer=False, overwrite=True)
    print(t)
...
def graph(data):
    for i in range(25):
        ...
        plt.bar(bar_spacing, counts, width=0.75, color='tab:orange')
        for j in range(7):
            plt.text(x=bar_spacing[j]-0.2, y=counts[j]+6, s=counts[j], fontsize=12)
        plt.xlabel('Rankings', fontsize=14)
        plt.ylabel('Frequencies', fontsize=14)
        plt.title('Rankings for Survey %s (Response Time > 100 Seconds)' % index[0], fontsize=14)
        ...
        plt.savefig('Figures_dropped_100/%s_ranking_bar_plot_dropped_100.png' % index[0], overwrite=True)
        plt.show()
